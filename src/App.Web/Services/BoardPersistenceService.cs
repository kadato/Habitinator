...
public sealed class BoardPersistenceService
{
    ...
    public async Task SeedBoardDataIfMissingAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        _dbContext.BoardItems.AddRange(
            ...
            new BoardItemEntity
            {
                ...
                Title = "Workout",
                IsCompleted = false,
                DailyStartDate = new DateTime(utcNow.UtcDateTime.Year, utcNow.UtcDateTime.Month, utcNow.UtcDateTime.Day,
            ...
            new BoardItemEntity
            {
                ...
                DailyRepeatType = (int)DailyRepeatType.Daily,
                DailyRepeatInterval = 1,
                DailyLastCompletedOn = new DateTime(utcNow.UtcDateTime.Year, utcNow.UtcDateTime.Month,
                    utcNow.UtcDateTime.Day, 0, 0, 0, DateTimeKind.Utc),
                CreatedAtUtc = utcNow,
                UpdatedAtUtc = utcNow
            },
            new BoardItemEntity
    ...
    {
        return ToModelWithToday(entity, DailySchedule.UtcToday);
    }

    private static BoardItem ToModelForDailyCheck(BoardItemEntity entity, DateOnly today)
    {
        return ToModelWithToday(entity, today);
    }
