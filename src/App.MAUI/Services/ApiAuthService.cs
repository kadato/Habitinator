...
public sealed class ApiAuthService
{
    ...
    {
        HttpClient client = _http.CreateClient("apiAuth");
        using HttpResponseMessage res = await client.PostAsJsonAsync("api/auth/login", request, Serializer, cancellationToken);
    ...
    {
        HttpClient client = _http.CreateClient("apiAuth");
        using HttpResponseMessage res = await client.PostAsync("api/auth/guest-jwt", null, cancellationToken);
        if (!res.IsSuccessStatusCode)
        {
            return null;
        }
    ...
    {
        HttpClient client = _http.CreateClient("apiAuth");
        using HttpResponseMessage res = await client.PostAsJsonAsync("api/auth/register", request, Serializer, cancellationToken);
