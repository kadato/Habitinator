...
public sealed class ActivityStatisticsService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        (string key, DateOnly start, DateOnly end) = ResolveActivityPeriod(periodKey, utcToday, options);
        ...
        var rows = await _db.UserActivityEvents.AsNoTracking()
            .Where(e => e.UserId == userId && e.OccurredAtUtc >= fromUtc && e.OccurredAtUtc < toUtc)
            .Select(e => new { e.OccurredAtUtc, e.EventType, e.DurationSeconds, e.BoardItemId })
            .ToListAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        DateOnly utcToday = DailySchedule.UtcToday;
        IReadOnlyList<DailyGraphPeriodOption> options = await BuildDailyPeriodOptionsAsync(userId, utcToday, cancellationToken);
        (string key, DateOnly rangeStart, DateOnly rangeEnd) = ResolveActivityPeriod(periodKey, utcToday, options);
        ...
        var eventRows = await _db.UserActivityEvents.AsNoTracking()
            ...
            .Select(e => new { e.OccurredAtUtc, e.BoardItemId })
            .ToListAsync(cancellationToken);
        eventRows = eventRows
            .Where(e => e.BoardItemId.HasValue && dailyIds.Contains(e.BoardItemId.Value))
            .ToList();
    ...
    private static (string Key, DateOnly Start, DateOnly End) ResolveActivityPeriod(
        string? periodKey,
        DateOnly utcToday,
    ...
    private List<ActivityHeatmapCellDto> BuildRangeContributionHeatmap(
        DateOnly rangeStart,
        DateOnly rangeEnd,
        DateOnly utcToday,
